...
print("=== 登录 ===")
login_resp = session.post(f"{BASE_URL}/auth/api/login", json={
    "username": "rui0908",
    "password": "rui0908"
})
...
if login_resp.status_code != 200:
    print("登录失败，尝试注册...")
    reg_resp = session.post(f"{BASE_URL}/auth/api/register", json={
        "username": "rui0908",
        "password": "rui0908",
...
# 发布文章
print("\n=== 发布文章 ===")
create_resp = session.post(f"{BASE_URL}/article/api", json=article_data)
print(f"发布状态: {create_resp.status_code}")
